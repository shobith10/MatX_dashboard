import 'package:dashboard_application/src/components/styles/colors.dart';
import 'package:flutter/cupertino.dart';

TextStyle style30white =
    TextStyle(fontSize: 30, fontWeight: FontWeight.w800, color: whiteColor);

TextStyle style23white =
    TextStyle(fontSize: 23, fontWeight: FontWeight.w600, color: whiteColor);
TextStyle style20whiteNotbold =
    TextStyle(fontSize: 20, fontWeight: FontWeight.w400, color: whiteColor);

TextStyle style25whitebold =
    TextStyle(fontSize: 25, fontWeight: FontWeight.w600, color: whiteColor);

TextStyle style18white =
    TextStyle(fontSize: 18, fontWeight: FontWeight.w400, color: whiteColor);

TextStyle style15greyLite = TextStyle(
    fontSize: 15,
    fontWeight: FontWeight.w400,
    color: geryColor.withOpacity(0.9));

TextStyle textfieldTextStyle = TextStyle(
  color: blackColor,
  fontWeight: FontWeight.w500,
  letterSpacing: 0.75,
  fontSize: 12,
);

TextStyle hintStyle = TextStyle(
  color: hintColor,
  fontWeight: FontWeight.w300,
  fontSize: 12,
);
